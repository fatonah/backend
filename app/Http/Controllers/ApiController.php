	...
	public function debug(){
        ...
        foreach ($sendtrans as $strans ) {
            ...
            if($checksendtx == 0){
                ...
                $txdet = json_decode(file_get_contents('https://api.blockchair.com/bitcoin/dashboards/transaction/'.$strans['txid']));
                if($txdet->data) {
		...
		$txid = "3bbc0365287833b12e2fa5b3b9e825480e2a2e0b689ae300bad16532d623a857";
		$txdet = json_decode(file_get_contents('https://api.blockchair.com/bitcoin/dashboards/transaction/'.$txid), TRUE);
		dd(getblocktime('BTC', '00000000000000000014fcb29e6e3b0ead3bd2e307d7f619a935f1d5323e9013'));
		dd(getblockhash('BTC', 593468));
		dd($txdet);
		dd(listransaction('BTC', 'usr_bsod666', 'myr', 'bitcoin'));
	...
	public function reset_password(Request $request){
		...
		if(isset($user)){
			...
			$email_msj = $user->username.'<p> We received a request to reset your DORADO password, please click link below.</p><p><a href="'.settings('url').'password/reset/'.$hash.'"  style="display: inline-block; padding: 11px 30px; margin: 20px 0px 30px; font-size: 15px; color: #fff; background: #4fc3f7; border-radius: 60px; text-decoration:none;">Reset Pasword</a></p>';
		  	...
		  	send_reset_password($user->email, 'DORADO Account Reset Password', $user->username, $hash); 
